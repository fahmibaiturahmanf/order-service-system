...
// FUNGSI UNTUK DASHBOARD DAN REKAPITULASI UNTUK ADMIN
const getPendingPaymentsCount = async (req, res) => {
    try {
        ...
        console.error("Error getting pending payments count for admin dashboard:", error);
...
const getMonthlyOrdersCount = async (req, res) => {
    try {
        ...
        console.error("Error getting monthly orders count for admin dashboard:", error);
...
const getTotalUsersCount = async (req, res) => {
    try {
        ...
        console.error("Error getting total users count for admin dashboard:", error);
...
const getRecentOrders = async (req, res) => {
    try {
        ...
        console.error("Error getting recent orders for admin dashboard:", error);
...
const getDashboardRecap = async (req, res) => {
    try {
        ...
        console.error("Error saat mendapatkan data rekap dashboard admin:", error);
...
const getRecapHtml = async (req, res) => {
    ...
    try {
        ...
        // Render view 'rekap.ejs'. Pastikan Anda memiliki file ini di 'Backend/views/'
        res.render('rekap', {
        ...
        console.error('Error saat membuat rekap HTML untuk admin dashboard:', err);
...
const generatePdfRecap = async (req, res) => {
    ...
    try {
        ...
        if (!token) {
            console.error("Token autentikasi tidak ditemukan saat mencoba generate PDF untuk admin dashboard.");
        ...
        // URL ini harus sesuai dengan endpoint rute HTML recap di server Anda
        const urlToRender = `http://localhost:${process.env.PORT || 5000}/api/admin-dashboard/rekap-html/${year}/${month}`;
        ...
        if (pageContent.includes("Akses ditolak") || pageContent.includes("Unauthorized") || pageContent.includes("Forbidden") || pageContent.includes("Error")) {
            console.error("Puppeteer gagal mendapatkan konten rekap HTML untuk admin dashboard. Kemungkinan masalah autentikasi atau error server di rute HTML.");
            console.error("Konten Halaman Error (sebagian):", pageContent.substring(0, 500));
            await browser.close();
            return res.status(403).json({
                message: "Gagal membuat PDF: Akses ditolak. Membutuhkan peran admin."
            });
        }
        ...
        console.error('Error generating PDF for admin dashboard (controller):', err);
...
const getMonthlyOrderTrends = async (req, res) => {
    try {
        ...
        console.error("Error getting monthly order trends for admin dashboard:", error);
...
const getOrderStatusDistribution = async (req, res) => {
    try {
        ...
        console.error("Error getting order status distribution for admin dashboard:", error);
